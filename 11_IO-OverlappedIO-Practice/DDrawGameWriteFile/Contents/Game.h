...
struct START_ARGS
{
	FILE_WRITER_TYPE type;
};
class CGame
{
	CDDrawDevice*	m_pDDraw = nullptr;
	IFileWriter*	m_pFileWriter = nullptr;
	HWND	m_hWnd = nullptr;
	...
	DWORD m_dwFileWritingCount = 0;
	WCHAR m_wchWritingModeText[128] = {};
	DWORD m_dwWritingModeTextLen = 0;
	DWORD m_dwWritingModeTexColor = 0;
	LARGE_INTEGER	m_PrvCounterForUpdateText = {};
...
//	int m_iPlayerPosY = 0;
	...
	void ParseCmdLine(START_ARGS* pOutArg, const WCHAR* wchSrcCmdLine);
	void UpdateWritingModeInfo();
...
public:
	...
	BOOL	Initialize(HWND hWnd, const WCHAR* pCmdLine);
