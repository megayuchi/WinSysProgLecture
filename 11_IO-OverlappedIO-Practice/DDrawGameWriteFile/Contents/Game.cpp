...
#include "../FileWriter/FileWriterSync.h"
#include "../FileWriter/FileWriterSyncMT.h"
#include "../FileWriter/FileWriterAsync.h"
...
BOOL CGame::Initialize(HWND hWnd,const WCHAR* pCmdLine)
{
	...
	const UINT64 MAX_FILE_SIZE = 8ull * 1024ull * 1024ull * 1024ull;	// 8GB

	START_ARGS args = {};
	ParseCmdLine(&args, pCmdLine);

	switch (args.type)
	{
		case FILE_WRITER_TYPE_SYNC:
			m_pFileWriter = new CFileWriterSync;
			break;
		case FILE_WRITER_TYPE_SYNC_MT:
			m_pFileWriter = new CFileWriterSyncMT;
			break;
		case FILE_WRITER_TYPE_ASYNC:
			m_pFileWriter = new CFileWriterAsync;
			break;
		default:
			__debugbreak();
	}
	m_pFileWriter->Initialize(MAX_FILE_SIZE);	
...
void CGame::Process()
{
	...
	float	fElpasedTickForUpdateText = QCMeasureElapsedTick(CurCounter, m_PrvCounterForUpdateText);
	if (fElpasedTickForUpdateText > 500.0f)
	{
		UpdateWritingModeInfo();
		m_PrvCounterForUpdateText = CurCounter;
	}
	DrawScene();
}
void CGame::UpdateWritingModeInfo()
{
	// update info text for writing mode
	if (m_bWriteToFile)
	{
		m_dwWritingModeTexColor = 0xffffffff;
		const WCHAR* wchStatus = L"";
		if (m_pFileWriter->IsWriting())
		{
			m_dwFileWritingCount++;
			m_dwFileWritingCount = m_dwFileWritingCount % 2;
			if (m_dwFileWritingCount)
			{
				m_dwWritingModeTexColor = 0xffff0000;
			}
			else
			{
				m_dwWritingModeTexColor = 0xffffff00;
			}
			wchStatus = L"Writing...";
		}
		else
		...
		const WCHAR* wchType = L"";
		FILE_WRITER_TYPE type = m_pFileWriter->GetType();
		switch (type)
		{
			case FILE_WRITER_TYPE_SYNC:
				wchType = L"Sync";
		...
		m_dwWritingModeTextLen = swprintf_s(m_wchWritingModeText, L"Recording Mode Enabled(%s) - %s", wchType, wchStatus);
	}
	else
	{
		m_dwWritingModeTextLen = 0;
	}
}
...
void CGame::DrawScene()
{
	...
	m_pDDraw->BeginDraw(m_bWriteToFile, m_pFileWriter);
...
//	m_pDDraw->DrawImageData(m_iMidScrollImagePosX, m_iMidScrollImagePosY, m_pMidScrollImageData);
	...
	if (m_pDDraw->BeginGDI(&hDC))
	{
		...
		if (m_bWriteToFile)
		{	
			m_pDDraw->WriteText(m_wchWritingModeText, m_dwWritingModeTextLen, 0, 20, m_dwWritingModeTexColor, hDC);
		}
		m_pDDraw->EndGDI(hDC);
	}
	m_pDDraw->OnDraw();
...
void CGame::ToggleWriteToFile()
{
	m_bWriteToFile = (m_bWriteToFile == 0);
	...
	if (m_bWriteToFile)
	{
		...
		m_pFileWriter->Begin(wchFileName);
	}
	else
	{
		m_pFileWriter->Stop();
...
void CGame::ParseCmdLine(START_ARGS* pOutArg, const WCHAR* wchSrcCmdLine)
{
	//wcscpy_s(wchCmdBuffer, wchSrcCmdLine);
	WCHAR wchCmdBuffer[256] = {};
	const DWORD dwMaxLen = (DWORD)_countof(wchCmdBuffer);
	if (!dwMaxLen)
	{
		__debugbreak();
		return;
	...
	DWORD	dwCmdLen = (DWORD)wcslen(wchSrcCmdLine);
	if (!dwCmdLen)
	{
		return;
	}

	if (dwCmdLen > dwMaxLen - 1)
	{
		dwCmdLen = dwMaxLen - 1;
	}
	memcpy(wchCmdBuffer, wchSrcCmdLine, sizeof(WCHAR) * dwCmdLen);
	wchCmdBuffer[dwCmdLen] = 0;

	WCHAR*	pNextContext = nullptr;
	WCHAR*	pArg = wcstok_s(wchCmdBuffer, L" ", &pNextContext);

	while (pArg != nullptr)
	{
		//if (!strcmp( pArg, "/ingameui"))
		//{
		//	pCmd->bGameUI = TRUE;
		//	goto lb_next;
		//}
		const DWORD	MAX_ARG_LEN = 16;

		DWORD	dwArgLen = (DWORD)wcslen(pArg);
		if (!dwArgLen || dwArgLen >= MAX_ARG_LEN)
		{
			goto lb_next;
		}
		if (!_wcsicmp(pArg, L"/sync"))
		{	
			pOutArg->type = FILE_WRITER_TYPE_SYNC;
			goto lb_next;
		}
		if (!_wcsicmp(pArg, L"/syncmt"))
		{
			pOutArg->type = FILE_WRITER_TYPE_SYNC_MT ;
			goto lb_next;
		}
		if (!_wcsicmp(pArg, L"/async"))
		{
			pOutArg->type = FILE_WRITER_TYPE_ASYNC;	
			goto lb_next;
		}
	lb_next:
		pArg = wcstok_s(nullptr, L" ", &pNextContext);
	}
}
CGame::~CGame()
{
