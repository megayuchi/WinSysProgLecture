...
BOOL CDDrawDevice::BeginDraw(BOOL bWriteToFile, IFileWriter* pFileWriter)
{
	BOOL	bResult = FALSE;
	...
	bResult = TRUE;
	m_bUseWriteToFile = bWriteToFile;
lb_return:
	return bResult;
}
...
void CDDrawDevice::EndDraw()
{
	if (m_bUseWriteToFile)
	{
		...
		m_pCurFileWriter->Write(m_pWriteBuffer, m_dwWidth, m_dwHeight, m_dwWriteBufferPitch);
	}
	else
	{
		UnlockBackBuffer();
