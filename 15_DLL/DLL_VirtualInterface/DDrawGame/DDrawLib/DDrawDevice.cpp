...
BOOL CDDrawDevice::CreateBackBuffer(DWORD dwWidth, DWORD dwHeight)
{
	...
	// post effect
	m_dwScratchBufferWidth = m_dwWidth;
	m_dwScratchBufferHeight = m_dwHeight;
	m_pScratchBuffer = (char*)_aligned_malloc(m_dwWidth * m_dwHeight * 4, 32);
	memset(m_pScratchBuffer, 0, m_dwWidth * m_dwHeight * 4);
...
BOOL CDDrawDevice::BeginDraw(CSharedMemoryWriter* pMemoryWriter)
{
	BOOL	bResult = FALSE;

	if (pMemoryWriter)
	{
		m_pWriteBuffer = m_pScratchBuffer;
	...
	m_pCurMemoryWriter = pMemoryWriter;
	bResult = TRUE;
...
void CDDrawDevice::EndDraw()
{
	if (m_pCurMemoryWriter)
	{
		...
		char* pSrc = m_pWriteBuffer;
		char* pDest = pBuffer;
		for (DWORD y = 0; y < m_dwHeight; y++)
		{
			memcpy(pDest, pSrc, m_dwWidth * 4);
			pSrc += m_dwWriteBufferPitch;
			pDest += dwPitch;
		}
		UnlockBackBuffer();
		
		m_pCurMemoryWriter->Write(m_pWriteBuffer, m_dwWidth, m_dwHeight, m_dwWriteBufferPitch);
	}
	else
	{
		UnlockBackBuffer();
...
void CDDrawDevice::CleanupBackBuffer()
{
	if (m_pScratchBuffer)
	{
		_aligned_free(m_pScratchBuffer);
		m_pScratchBuffer = nullptr;
		m_dwScratchBufferWidth = 0;
		m_dwScratchBufferHeight = 0;
	}
