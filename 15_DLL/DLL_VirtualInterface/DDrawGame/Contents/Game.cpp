...
#include "../../SharedMemoryWriter/SharedMemoryWriter.h"
...
BOOL CGame::Initialize(HWND hWnd,const WCHAR* pCmdLine)
{
	...
	m_pMemoryWriter = new CSharedMemoryWriter;
	m_pMemoryWriter->Initialize(L"DDrawGame-MemoryMap", L"DDrawGame-Mutex", 4096, 4096);
...
void CGame::Process()
{
	...
	if (fElpasedTick > m_fTicksPerGameFrame)
	{
		...
		if (m_pNamedPipeServer)
		{
			m_pNamedPipeServer->Run(&bOnConnected, &bMsgExist);
			if (bMsgExist)
			{
				ProcessMsg();
			}
...
void CGame::ProcessMsg()
{
	...
	const char* pPacket = nullptr;
	DWORD dwPacketSize = 0;
	while (pPacket = m_pNamedPipeServer->DispatchMsgSeq(&dwPacketSize))
	{
		PIPE_PACKET_PROTOCOL ptcl = (PIPE_PACKET_PROTOCOL)(*(DWORD*)pPacket);
		switch (ptcl)
		{
			case PIPE_PACKET_PROTOCOL_ON_KEYDOWN:
				{
					PACKET_ON_KEYDOWN* pPacketOnKeyDown = (PACKET_ON_KEYDOWN*)pPacket;
					OnKeyDown(pPacketOnKeyDown->Key, pPacketOnKeyDown->ScanCode);
				}
				break;
			case PIPE_PACKET_PROTOCOL_ON_KEYUP:
				{
					PACKET_ON_KEYUP* pPacketOnKeyUp = (PACKET_ON_KEYUP*)pPacket;
					OnKeyUp(pPacketOnKeyUp->Key, pPacketOnKeyUp->ScanCode);
				}
				break;
			case PIPE_PACKET_PROTOCOL_STRING:
				{
					BOOL bFound = FALSE;
					PACKET_STRING* pPacketString = (PACKET_STRING*)pPacket;
					DWORD dwLen = (DWORD)wcslen(pPacketString->wchTxt);
					if (dwLen != pPacketString->dwTxtLen)
						__debugbreak();

					for (DWORD i = 0; i < dwTextArgCount; i++)
					{
						if (!wcscmp(pPacketString->wchTxt, wchTxtArgList[i]))
						{
							bFound = TRUE;
							break;
						}
	...
	m_pNamedPipeServer->ClearMsgBuffer();
}
...
void CGame::DrawScene()
{
	...
	m_pDDraw->BeginDraw(m_pMemoryWriter);
...
void CGame::Cleanup()
{
	...
	if (m_pMemoryWriter)
	{
		delete m_pMemoryWriter;
		m_pMemoryWriter = nullptr;
	}
	if (m_pDDraw)
	{
		delete m_pDDraw;
...
BOOL CGame::InitNamedPipeLib()
{
	...
	pCreateFunc(&m_pNamedPipeServer);
	m_pNamedPipeServer->InitializeAsServer(512, 512, 1024 * 1024);

	return TRUE;
}
void CGame::CleanupNamedPipeLib()
{
	if (m_pNamedPipeServer)
	{
		m_pNamedPipeServer->Release();
		m_pNamedPipeServer = nullptr;
	}
	if (m_hNamedPipeLibDLL)
	{
