...
BOOL CNamedPipe::InitializeAsServer(DWORD dwRecvBufferSize, DWORD dwSendBufferSize, DWORD dwMaxMsgBufferSize)
{
	...
	// PIPE_ACCESS_DUPLEX -> 양방향 전송
	// FILE_FLAG_OVERLAPPED -> 비동기 모드
	// PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE -> 전송 단위 메시지. 기본적으로는 보낸만큼이 메시지로 간주되어서 메시지 단위로 수신됨. 쪼개지지 않음. 하지만 버퍼 사이즈가 부족하면 결과적으로 쪼개짐.
	// PIPE_TYPE_BYTE | PIPE_READMODE_BYTE -> 전송단위 바이트 스트림. 바이트 단위로 전송되므로 데이터는 언제나 쪼개질 수 있음.
...
void CNamedPipe::Run(BOOL* pbOutOnConnected, BOOL* pbOutPacketExist)
{
	...
	if (m_bIsConnected)
	{
		// 연결이 된 상태인데 아직 Recv(ReadFile호출)시도를 하지 않은 경우
		if (!m_bIsReceiving)
		{
			// Recv(ReadFile호출)시도
			Recv();
	...
	if (m_dwMsgCount)
	{
		// 현재 수신되어 처리를 대기하고 있는 완전한 메시지가 존재함
	...
	else
	{
		...
		else
		{
			if (ERROR_MORE_DATA == dwErr)
			{
				...
				// NamedPipe가 PIPE_READMODE_MESSAGE로 생성되었을때, 발생할 수 있다. 요청된 사이즈만 읽었을 경우 '메시지 단위로 읽는다는 규칙'이 깨지기 때문에 발생하는 에러이다.PIPE_READMODE_BYTE이면 발생하지 않는다.
