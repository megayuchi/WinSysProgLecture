					  ...
					  _In_ int       nCmdShow)
{
	...
	// CCDrawViewer를 수동실행할 경우 StartDDrawViewProcess()를 주석처리한다. 이 경우 DDrawGame먼저, DDrawViewer를 나중에 실행할 것
...
BOOL StartDDrawViewProcess(HANDLE* phOutProcess, HANDLE* phOutThread)
{
	...
	#ifdef _DEBUG
		wchExeName = L"\\DDrawViewer_arm64_debug.exe";
	#else
		wchExeName = L"\\DDrawViewer_arm64_release.exe";
	#endif
#elif defined(_M_AMD64)
	#ifdef _DEBUG
		wchExeName = L"\\DDrawViewer_x64_debug.exe";
	#else
		wchExeName = L"\\DDrawViewer_x64_release.exe";
	#endif
#elif defined(_M_IX86)
	#ifdef _DEBUG
		wchExeName = L"\\DDrawViewer_x86_debug.exe";
	#else
		wchExeName = L"\\DDrawViewer_x86_release.exe";
	#endif
#endif
